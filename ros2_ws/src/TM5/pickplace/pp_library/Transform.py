...
class TransformClass():
    def __init__(self):
        ...
        pick_to_safepick_cup = TransformStamped()
        pick_to_safepick_cup.header.frame_id = 'pick'
        pick_to_safepick_cup.header.stamp = rclpy.time.Time().to_msg()
        pick_to_safepick_cup.child_frame_id = 'safepick_cup'
        pick_to_safepick_cup.transform.translation = Vector3(x = 0.0, y = -0.1, z = 0.0)
        pick_to_safepick_cup.transform.rotation = Quaternion(w = 1.0, x = 0.0, y = 0.0, z = 0.0)
        ...
        place_to_safeplace_cup = TransformStamped()
        place_to_safeplace_cup.header.frame_id = 'place'
        place_to_safeplace_cup.header.stamp = rclpy.time.Time().to_msg()
        place_to_safeplace_cup.child_frame_id = 'safeplace_cup'
        place_to_safeplace_cup.transform.translation = Vector3(x = 0.0, y = -0.1, z = 0.0)
        place_to_safeplace_cup.transform.rotation = Quaternion(w = 1.0, x = 0.0, y = 0.0, z = 0.0)

        self.tfBuffer.set_transform(base_to_home, "")
        self.tfBuffer.set_transform(pick_to_safepick, "")
        self.tfBuffer.set_transform(pick_to_safepick_cup, "")
        self.tfBuffer.set_transform(place_to_safeplace, "")
        self.tfBuffer.set_transform(place_to_safeplace_cup, "")
    ...
    def add_vbases(self, vbase_pick, vbase_place, vbase_number, vbase_offset):
        ...
        if (vbase_number == 0 and vbase_offset == False):
            vbase_to_pick.header.frame_id = 'vbase_pick'
        elif (vbase_number == 1 and vbase_offset == False):
            vbase_to_pick.header.frame_id = 'vbase_pick_1'
        elif (vbase_number == 2 and vbase_offset == False):
            vbase_to_pick.header.frame_id = 'vbase_pick_2'
        elif (vbase_number == 3 and vbase_offset == False):
            vbase_to_pick.header.frame_id = 'vbase_pick_3'
        elif (vbase_number == 0 and vbase_offset == True):
            vbase_to_pick.header.frame_id = 'vbase_pick_offset'
        elif (vbase_number == 1 and vbase_offset == True):
            vbase_to_pick.header.frame_id = 'vbase_pick_1_offset'
        elif (vbase_number == 2 and vbase_offset == True):
            vbase_to_pick.header.frame_id = 'vbase_pick_2_offset'
        elif (vbase_number == 3 and vbase_offset == True):
            vbase_to_pick.header.frame_id = 'vbase_pick_3_offset'
        else:
            vbase_to_pick.header.frame_id = 'vbase_pick'
    ...
    def get_picks(self, vbase_euler, vbase_name, cup):
        ...
        if cup == True:
            safepick_tstamped = self.tfBuffer.lookup_transform('base','safepick_cup', rclpy.time.Time(seconds=0))
        else:
    ...
    def get_places(self, vbase_euler, vbase_name, cup):
        vbase_quaternion = self.quaternion_from_euler(self.convert_units(vbase_euler))
        self.update_vbase(vbase_quaternion, vbase_name)
        place_tstamped = self.tfBuffer.lookup_transform('base', 'place', rclpy.time.Time(seconds=0))
        if cup == True:
            safeplace_tstamped = self.tfBuffer.lookup_transform('base','safeplace_cup', rclpy.time.Time(seconds=0))
        else:
